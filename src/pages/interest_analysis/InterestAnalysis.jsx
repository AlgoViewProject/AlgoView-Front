...
const InterestAnalysis = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://localhost:8080/api/stream/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (analysisId) {
        navigate(`/interest-analysis/result?analysisId=${analysisId}`);
      } else {
        setErrorMessage("서버에서 분석 ID를 받지 못했습니다.");
      }

    } catch (error) {
      console.error("Upload error:", error);
      setErrorMessage("실시간 분석 중 오류가 발생했습니다.");
    }
  };
